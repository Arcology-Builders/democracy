...
export class LoginBook extends Component {
  ...
  constructor(props) {
    ...
    const savedLogins = savedLoginString ? fromJS ( JSON.parse(savedLoginString) ) : List([])
    ...
    const hasSavedLogins = List.isList(savedLogins) && savedLogins.count() > 0 
    this.state = {
      logins          : savedLogins || List([]),
  ...
  async submitPay(evt) {
    await this.props.wallet.pay({
      weiValue : toWei(this.state.payAmount, 'ether'),
            ...
            return (
              ...
              >
                ...
                <div className="login loginBalance">
                  {this.props.getBalance(pair.get('address')).toString()}
                </div>
              </li>
            )}
          )}
