...
import { TYPES, createArgListTransform, createTransformFromMap,
  deployerTransform, runTransforms } from 'demo-transform'
...
describe('Pipelines', () => {
  ...
  it('reports input type mismatch', async () => {
    ...
    expect(result).to.be.rejectedWith('Arg named x mismatched type integer with error string Arg undefined did not have type integer')
  ...
  it('reports ', async () => {
    ...
    expect(result).to.be.rejectedWith('Transform run error in pipe 2 (indexed from 0) named main.\nblammo')
